...
export default function PostCard({ post, userData, userName, avatar }) {
  ...
  useEffect(() => {
    if (!userData?.bookmarks || !post?._id) return;
    const isBookmarked = userData.bookmarks.some(
      (id) => id === post._id || id === post._id.toString()
    );
  ...
  const handleToggleBookmark = async () => {
    if (!userData?._id) {
      return showAlert("error", "User not loaded yet");
    }
    ...
    try {
      if (!bookmarked) {
        ...
        await axios.delete(
          `${BACKEND_URL}/bookmarks/${userData._id}/${post._id}`
        );
        setBookmarked(false);
  ...
  const prevImage = () =>
    setModalIndex((i) => (i === 0 ? post?.images.length - 1 : i - 1));
  const nextImage = () =>
    setModalIndex((i) => (i === post?.images.length - 1 ? 0 : i + 1));
  ...
  const handleAddComment = async () => {
    if (!commentText.trim() || !userData?._id) return;
    try {
      const res = await axios.post(
        `${BACKEND_URL}/posts/${post._id}/comments`,
        {
          userId: userData._id,
          userName,
          avatar,
  ...
  const handleAddReply = async (commentId) => {
    ...
    if (!text?.trim() || !userData?._id) return;
    try {
      const res = await axios.post(
      ...
      <div className="flex items-center justify-between text-gray-600 mb-2">
        ...
        {userData?._id && (
          ...
          >
            <Bookmark
              className={`w-5 h-5 ${
                bookmarked ? "text-blue-600 fill-blue-600" : ""
              }`}
            />
